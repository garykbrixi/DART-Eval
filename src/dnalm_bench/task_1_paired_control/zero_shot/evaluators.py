...
class EvoEvaluator(ZeroShotPairedControlEvaluator, CausalZeroShotScore):
    def __init__(self, model_name, dataset, batch_size, num_workers, device):
    ...
    def model_fwd(self, tokens_in, attention_mask, tokens_out):
        with torch.inference_mode():
            ...
            # Convert to log probabilities and ensure float32 type
    ...
    def score(self, tokens, starts, ends, attention_mask):
        ...
        lls = self.model_fwd(tokens, attention_mask, tokens)
        clip_mask = torch.zeros_like(lls, dtype=torch.float32, device=self.device)
        
        for i in range(lls.shape[1]):
            clip_mask[:,i] = ((i >= starts) & (i < ends)).to(torch.float32)

        # Ensure everything is float32 before numpy conversion
        out = (lls * clip_mask).sum(1).to(torch.float32).cpu().numpy()
...
#             for seqs, ctrls, inds in tqdm(self.dataloader, disable=(not progress_bar), ncols=120):
#                 seqs_str = onehot_to_chars(seqs)
#                 ctrls_str = onehot_to_chars(ctrls)

#                 seq_scores = np.array(score_sequences(seqs_str, self.model, self.tokenizer, reduce_method='sum'))
#                 ctrl_scores = np.array(score_sequences(ctrls_str, self.model, self.tokenizer, reduce_method='sum'))
